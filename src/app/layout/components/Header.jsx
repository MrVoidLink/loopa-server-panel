...
function Header({ setIsOpen }) {
  ...
  const triggerDeploy = async () => {
    ...
    try {
      const response = await authFetch("/api/deploy", { method: "POST" });
      let payload = null;
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-end gap-4 text-gray-300">
          ...
          <div className="relative">
            ...
            {showAccountMenu && (
              <div className="absolute right-0 mt-3 w-64 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)]/95 shadow-[0_20px_45px_rgba(15,23,42,0.35)] backdrop-blur">
                <div className="px-4 py-3 border-b border-[var(--border-color)]/60 text-left">
                  <p className="text-[10px] uppercase tracking-[0.2em] text-[var(--text-muted)]">
                    Signed in as
                  </p>
                ...
                <button
                  ...
                  className="flex w-full items-center gap-2 px-4 py-3 text-sm text-[var(--text-main)] hover:bg-[var(--bg-main)]/40 transition"
                >
                  <KeyRound size={16} />
                ...
                <button
                  ...
                  className="flex w-full items-center gap-2 px-4 py-3 text-sm text-red-400 hover:bg-red-500/10 transition"
                >
                  <LogOut size={16} />
                  <span>Sign out</span>
                </button>
          ...
          >
            <div
              className={`absolute top-[3px] left-[3px] w-5 h-5 rounded-full transition-all duration-300 ${
                darkMode ? "translate-x-0 bg-emerald-400" : "translate-x-6 bg-cyan-400"
              }`}
            />
      ...
      <ChangePasswordModal
        ...
        onSuccess={() => {
          ...
          addNotification({
            type: "success",
            message: "Password updated. Sign in again with the new credentials.",
          });
          logout();
        }}
      />
